...
const lights = { "res": 2048, "num": 4, "frbuf": [], "tex": [], "proj": [], "pos": [[-1, 2.9, -5.3], [0, 2.9, 0], [-3, 2.9, -3], [-5, 2.9, 0]], "look": [[-1, 0, -5], [0, 0, 0.3], [-3, 0, -3.3], [-5.3, 0, 0]] }

for (let i = 0; i < lights["num"]; i++) {
    let lightData = createLight(gl, lights["res"], 160.0);
    lights["frbuf"][i] = lightData[0];
...
function renderDepthMap() {
    for (let i = 0; i < lights["num"]; i++) {
        prepareGLState(gl, lights["res"], lights["res"], shadowProgram, lights["frbuf"][i], gl.BACK);

        gl.uniformMatrix4fv(uniformLocationsShadow["lightViewMatrix"], false, lookAtMat4f(lights["pos"][i], lights["look"][i], [0, 1, 0]));
        gl.uniformMatrix4fv(uniformLocationsShadow["lightProjectionMatrix"], false, lights["proj"][i]);
        gl.uniform3fv(uniformLocationsShadow["lightPosition"], lights["pos"][i]);
...
function renderScene() {
    ...
    for (let i = 0; i < lights["num"]; i++) {
        gl.uniformMatrix4fv(uniformLocations["lightViewMatrices"][i], false, lookAtMat4f(lights["pos"][i], lights["look"][i], [0, 1, 0]));
        gl.uniformMatrix4fv(uniformLocations["lightProjectionMatrices"][i], false, lights["proj"][i]);
