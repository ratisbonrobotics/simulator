...
for (let i = 0; i < numLights; i++) {
    ...
    lightProjectionMatrices[i] = orthoMat4f(-20, 20, 20, -20, 0.01, 10000);
}
...
const fragmentshadersource = `
    ...
    void main() {
        ...
        float shadow = 1.0;
        for (int i = 0; i < ` + numLights + `; i++) {
            shadow = min(shadow, calculateShadow(o_shadowCoords[i], shadowTextures[i], lightPositions[i]));
        }
...
function renderDepthMap() {
    for (let i = 0; i < numLights; i++) {
        prepareGLState(gl, shadowMapResolution, shadowMapResolution, shadowProgram, shadowFramebuffers[i], gl.FRONT);

        lightViewMatrices[i] = lookAtMat4f(lightPositions[i], [0, 0, 0], [0, 1, 0]);
