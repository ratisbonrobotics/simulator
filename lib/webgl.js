...
function addAndActivateTexture(gl, uniform_location, source) {
    let img = new Image();
    img.src = source;
    let texture = gl.createTexture();
    let idx = Object.keys(textures).length;
...
function activateTexture(gl, uniform_location, texture) {
    let idx = textures[texture];
    if (idx !== undefined) {
        gl.activeTexture(gl.TEXTURE0 + idx);
        gl.uniform1i(uniform_location, idx);
    } else {
