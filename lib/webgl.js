...
function drawDrawable(gl, drawable, attribLocations, uniformLocations, textured) {
    ...
    gl.uniformMatrix4fv(uniformLocations["modelmatrix"], false, drawable["modelmatrix"]);
    for (let p = 0; p < drawable["vertexbuffer"].length; p++) {
        connectBufferToAttribute(gl, gl.ARRAY_BUFFER, drawable["vertexbuffer"][p]["verticies"], attribLocations.vertexposition, 3);
        if (textured) {
            connectBufferToAttribute(gl, gl.ARRAY_BUFFER, drawable["texcoordbuffer"][p], attribLocations.texturecoordinate, 2);
            connectBufferToAttribute(gl, gl.ARRAY_BUFFER, drawable["normalbuffer"][p], attribLocations.vertexnormal, 3);
...
function createLight(gl, res, fov) {
    ...
    return [shadowFramebuffer, shadowTexture, perspecMat4f(degToRad(fov), 1.0, 0.0001, 1000)];
}
