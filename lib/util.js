...
async function parseGLB(filename) {
  ...
  for (let i = 0; i < gltf.scenes.length; i++) {
    ...
    for (let j = 0; j < gltf.scenes[i].nodes.length; j++) {
      ...
      visitAllChildren(node, gltf, 0);
    }
  }

}

function visitAllChildren(node, gltf, depth) {

  for (let k = 0; k < node.children.length; k++) {
    let child = gltf.nodes[node.children[k]];
    console.log("Depth: ", depth, "Node Child name: ", child.name);

    if (child.children !== undefined) visitAllChildren(child, gltf, depth + 1);
...
function prepareGLTFSceneForRendering(filename) {

}

function prepareGLTFNodeForRendering(filename) {
