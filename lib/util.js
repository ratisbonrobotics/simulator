...
async function parseGLB(filename) {
  ...
  console.log(getVertexDataForNode(1, parsed_gltf));
...
function parseGLTF(gltf, binaryChunk) {
  ...
  const accessorData = accessors.map(accessor => {
    ...
    const data = new TypedArray(
      ...
      bufferViewArray.byteOffset + (byteOffset || 0),
      size
    );
...
function getVertexDataForNode(nodeIndex, gltf) {
  const node = gltf.nodes[nodeIndex];
  const mesh = node.mesh;

  if (mesh === undefined) {
    return []; // Node does not have a mesh
  }

  const vertexData = [];

  for (const primitive of mesh.primitives) {
    const positionAccessor = primitive.attributeData.POSITION;
    const { data, componentType, count, numComponents } = positionAccessor;

    for (let i = 0; i < count; i++) {
      const vertex = [];
      for (let j = 0; j < numComponents; j++) {
        vertex.push(data[i * numComponents + j]);
      }
