...
async function parseGLB(filename) {
  ...
  console.log(parsed_gltf.nodes);

  const preprocessedVertices = [];

  for (let l = 0; l < parsed_gltf.nodes.length; l++) {
    if (parsed_gltf.nodes[l].mesh === undefined) continue;
    console.log(l);
    for (k = 0; k < parsed_gltf.nodes[1].mesh.primitives.length; k++) {
      const primitive = parsed_gltf.nodes[1].mesh.primitives[k];
      ...
      for (let i = 0; i < indicesData.length; i++) {
        ...
        for (let j = 0; j < numComponents; j++) {
          preprocessedVertices.push(positionData[index * numComponents + j]);
        }
      }
    }
  }

  return new Float32Array(preprocessedVertices);
}
