...
async function parseGLB(filename) {
    ...
    console.log(parseGLTF(gltf, binaryChunk));
...
function parseGLTF(gltf, binaryChunk) {
    ...
    const bufferViewData = bufferViews.map(bufferView => {
      ...
      return new Uint8Array(binaryChunk.buffer, byteOffset, byteLength);
    });
  
    // Parse accessors
    const accessorData = accessors.map(accessor => {
      const { bufferView, byteOffset, componentType, count, type } = accessor;
      const bufferViewArray = bufferViewData[bufferView];
      const byteStride = accessor.byteStride || 0;
  
      let data;
      switch (componentType) {
        case 5120: data = new Int8Array(bufferViewArray.buffer, bufferViewArray.byteOffset + byteOffset, count); break;
        case 5121: data = new Uint8Array(bufferViewArray.buffer, bufferViewArray.byteOffset + byteOffset, count); break;
        case 5122: data = new Int16Array(bufferViewArray.buffer, bufferViewArray.byteOffset + byteOffset, count); break;
        case 5123: data = new Uint16Array(bufferViewArray.buffer, bufferViewArray.byteOffset + byteOffset, count); break;
        case 5125: data = new Uint32Array(bufferViewArray.buffer, bufferViewArray.byteOffset + byteOffset, count); break;
        case 5126: data = new Float32Array(bufferViewArray.buffer, bufferViewArray.byteOffset + byteOffset, count); break;
    ...
    const materialData = materials.map(material => {
      const { pbrMetallicRoughness, normalTexture, occlusionTexture, emissiveTexture, emissiveFactor, alphaMode, alphaCutoff, doubleSided } = material;
