...
async function parseMTL(filename) {
  ...
  for (const line of data) {
    if (line[0] == "newmtl") {
      ...
      materials[material_name]["map_Kd"].src = filename.split("/").slice(0, -1).join("/") + "/" + line[line.length - 1];
      if (line.includes("-o")) {
...
async function loadDrawable(obj_path, drawable) {
  let obj = await parseOBJ(obj_path);

  const texturePromises = Object.entries(obj).map(async ([_, value]) => {
    if (value["m"][0]["map_Kd"] && value["m"][0]["map_Kd"].src) {
      ...
      return createTexture(gl, colorImageURL);
    }
  });

  const textures = await Promise.all(texturePromises);

  for (const [index, [_, value]] of Object.entries(obj).entries()) {
