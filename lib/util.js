...
async function parseGLB(filename) {
  ...
  console.log(parsed_gltf);

  const textureMap = new Map();
  const preprocessedData = [];

  for (let l = 0; l < parsed_gltf.nodes.length; l++) {
    if (parsed_gltf.nodes[l].mesh === undefined) continue;
    for (k = 0; k < parsed_gltf.nodes[l].mesh.primitives.length; k++) {
      const primitive = parsed_gltf.nodes[l].mesh.primitives[k];
      const positionData = primitive.attributeData.POSITION.data;
      const texcoordData = primitive.attributeData.TEXCOORD_0?.data;
      const indicesData = primitive.indicesData.data;
      const numComponents = primitive.attributeData.POSITION.numComponents;
      const vertices = [];
      const texcoords = [];
      for (let i = 0; i < indicesData.length; i++) {
        const index = indicesData[i];
        for (let j = 0; j < numComponents; j++) {
          vertices.push(positionData[index * numComponents + j]);
        }
        if (texcoordData) {
          texcoords.push(texcoordData[index * 2], texcoordData[index * 2 + 1]);
        }
      }
      ...
      if (materialIndex !== null) {
        const material = parsed_gltf.materials[materialIndex];
        const textureIndex = material.pbrMetallicRoughness?.baseColorTexture?.index;
        if (textureIndex !== undefined && !textureMap.has(textureIndex)) {
          const texture = parsed_gltf.textures[textureIndex];
          if (texture && texture.imageData) {
            const bufferView = parsed_gltf.bufferViews[texture.imageData.bufferView];
            textureData = new Uint8Array(
              binaryChunk.buffer,
              binaryChunk.byteOffset + (bufferView.byteOffset || 0),
              bufferView.byteLength
            );
            const blob = new Blob([textureData], { type: texture.imageData.mimeType });
            textureURL = URL.createObjectURL(blob);
            textureMap.set(textureIndex, { textureData, textureURL });
          }
        } else if (textureIndex !== undefined) {
          const cachedTexture = textureMap.get(textureIndex);
          textureData = cachedTexture.textureData;
          textureURL = cachedTexture.textureURL;
        }
      }

      preprocessedData.push({
        v: new Float32Array(vertices),
        vt: texcoordData ? new Float32Array(texcoords) : null,
        t: textureData,
        tURL: textureURL
      });
    }
