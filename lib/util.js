...
function applyRotation(normal, rotation) {
...
function extract(child, gltf, binaryChunk, parentTranslation, parentRotation, parentScale, primitives) {
  ...
  for (let p = 0; p < mesh.primitives.length; p++) {
    ...
    let normalData = new Float32Array(
      binaryChunk.buffer,
      binaryChunk.byteOffset + normalBufferView.byteOffset,
    ...
    let normalVertexData = new Float32Array(indicesData.length * 3); // New array for normal data
    ...
    for (let i = 0; i < indicesData.length; i++) {
      ...
      normalVertexData[i * 3] = normal[0];
      normalVertexData[i * 3 + 1] = normal[1];
      normalVertexData[i * 3 + 2] = normal[2];
    }

    primitiveData.vertexData = vertexData;
    primitiveData.normalData = normalVertexData; // Store the normal data in primitiveData
